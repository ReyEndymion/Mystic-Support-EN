        ...
        for (let name in global.plugins) {
            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                            m.reply(`*[ ⚠️ 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝚁𝙴𝙿𝙾𝚁𝚃 𝚆𝙸𝚃𝙷 𝙵𝙰𝚄𝙻𝚃𝚂 ⚠️ ]*\n\n*—◉ 𝙿𝙻𝚄𝙶𝙸𝙽:* ${name}\n*—◉ 𝚄𝚂𝙴𝚁:* ${m.sender}\n*—◉ 𝙲𝙾𝙼𝙼𝙰𝙽𝙳:* ${m.text}\n\n*—◉ 𝙴𝚁𝚁𝙾𝚁:*\n\`\`\`${format(e)}\`\`\`\n\n*[❗] 𝚁𝙴𝙿𝙾𝚁𝚃 𝙸𝚃 𝚃𝙾 𝚃𝙷𝙴 𝙲𝚁𝙴𝙰𝚃𝙾𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃 𝚃𝙾 𝙶𝙸𝚅𝙴 𝙸𝚃 𝙰 𝚂𝙾𝙻𝚄𝚃𝙸𝙾𝙽, 𝚈𝙾𝚄 𝙲𝙰𝙽 𝚄𝚂𝙴 𝚃𝙷𝙴 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 #report*`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] 𝚈𝙾𝚄𝚁 𝙳𝙸𝙰𝙼𝙾𝙽𝙳𝚂 𝙰𝚁𝙴 𝚂𝙾𝙻𝙳 𝙾𝚄𝚃, 𝚈𝙾𝚄 𝙲𝙰𝙽 𝙱𝚄𝚈 𝙼𝙾𝚁𝙴 𝚄𝚂𝙸𝙽𝙶 𝚃𝙷𝙴 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 ${usedPrefix}buy <quantity>*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] 𝙻𝙴𝚅𝙴𝙻 𝚁𝙴𝚀𝚄𝙸𝚁𝙴𝙳 ${plugin.level} TO USE THIS COMMAND. YOUR LEVEL IS: ${_user.level}*`, m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*[ ⚠️  𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝚁𝙴𝙿𝙾𝚁𝚃 𝚆𝙸𝚃𝙷 𝙵𝙰𝚄𝙻𝚃𝚂 ⚠️ ]*\n\n*—◉ 𝙿𝙻𝚄𝙶𝙸𝙽:* ${m.plugin}\n*—◉ 𝚄𝚂𝚄𝙰𝚁𝙸𝙾:* ${m.sender}\n*—◉ 𝙲𝙾𝙼𝙼𝙰𝙽𝙳:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n*[❗] 𝚁𝙴𝙿𝙾𝚁𝚃 𝙸𝚃 𝚃𝙾 𝚃𝙷𝙴 𝙲𝚁𝙴𝙰𝚃𝙾𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃 𝚃𝙾 𝙶𝙸𝚅𝙴 𝙸𝚃 𝙰 𝚂𝙾𝙻𝚄𝚃𝙸𝙾𝙽, 𝚈𝙾𝚄 𝙲𝙰𝙽 𝚄𝚂𝙴 𝚃𝙷𝙴 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 #report*`.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
//this.sendButton(id, text, groupMetadata.subject, apii.data, [[(action == 'add' ? '💫 𝙱𝙸𝙴𝙽𝚅𝙴𝙽𝙸𝙳𝙾 💫' : '☠ 𝙰𝙳𝙸𝙾𝚂 ☠'), (action == 'add' ? '#welcomegc' : '#byegc')], ['♦ 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙲𝙾𝙼𝙼𝙰𝙽𝙳𝚂 ♦', `#menu`]], null, {mentions: this.parseMention(text)})
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙱𝚈 𝚃𝙷𝙴 𝙾𝚆𝙽𝙴𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃*',
        owner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙱𝚈 𝚃𝙷𝙴 𝙾𝚆𝙽𝙴𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃*',
        mods: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙱𝚈 𝙼𝙾𝙳𝙴𝚁𝙰𝚃𝙾𝚁𝚂 𝙰𝙽𝙳 𝚃𝙷𝙴 𝙾𝚆𝙽𝙴𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃*',
        premium: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙱𝚈 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 𝚄𝚂𝙴𝚁𝚂 𝙰𝙽𝙳 𝚃𝙷𝙴 𝙾𝚆𝙽𝙴𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃*',
        group: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙸𝙽 𝙶𝚁𝙾𝚄𝙿𝚂*',
        private: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙸𝙽 𝙿𝚁𝙸𝚅𝙰𝚃𝙴 𝙱𝙾𝚃 𝙲𝙷𝙰𝚃*',
        admin: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙲𝙰𝙽 𝙾𝙽𝙻𝚈 𝙱𝙴 𝚄𝚂𝙴𝙳 𝙱𝚈 𝙶𝚁𝙾𝚄𝙿 𝙰𝙳𝙼𝙸𝙽𝚂*',
        botAdmin: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙾 𝙱𝙴 𝙰𝙱𝙻𝙴 𝚃𝙾 𝚄𝚂𝙴 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙸𝚃 𝙸𝚂 𝙽𝙴𝙲𝙴𝚂𝚂𝙰𝚁𝚈 𝙵𝙾𝚁 𝚃𝙷𝙴 𝙱𝙾𝚃 𝚃𝙾 𝙱𝙴 𝙰𝙳𝙼𝙸𝙽, 𝙿𝚁𝙾𝙼𝙾𝚃𝙴 𝚃𝙷𝙸𝚂 𝙽𝚄𝙼𝙱𝙴𝚁 𝚃𝙾 𝙰𝙳𝙼𝙸𝙽*',
        unreg: '*[ 🛑 𝐇𝐄𝐘!! 𝐒𝐓𝐎𝐏, 𝐘𝐎𝐔 𝐀𝐑𝐄 𝐍𝐎𝐓 𝐑𝐄𝐆𝐈𝐒𝐓𝐄𝐑𝐄𝐃 🛑 ]*\n\n*—◉ 𝚃𝙾 𝚄𝚂𝙴 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝚈𝙾𝚄 𝙼𝚄𝚂𝚃 𝚁𝙴𝙶𝙸𝚂𝚃𝙴𝚁, 𝚄𝚂𝙴 𝚃𝙷𝙴 𝙲𝙾𝙼𝙼𝙰𝙽𝙳:*\n*➣ #𝚟𝚎𝚛𝚒𝚏𝚢 𝚗𝚊𝚖𝚎.𝚊𝚐𝚎*',
        restrict: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 𝚃𝙷𝙸𝚂 𝙲𝙾𝙼𝙼𝙰𝙽𝙳 𝙸𝚂 𝚁𝙴𝚂𝚃𝚁𝙸𝙲𝚃𝙴𝙳/𝙳𝙸𝚂𝙰𝙱𝙻𝙴𝙳 𝙱𝚈 𝙾𝚆𝙽𝙴𝚁 𝙾𝙵 𝚃𝙷𝙴 𝙱𝙾𝚃*'
    }[type]
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓', body: 'ᴛʜᴇ ᴍʏsᴛɪᴄ - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'https://github.com/BrunoSobrino/TheMystic-Bot-MD' }}}}, { quoted: m })
